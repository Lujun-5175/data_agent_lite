...
- **图表生成**：在聊天中直接请求图表，Agent 会根据字段与问题生成图片
- **结果内联展示**：生成后的图表会作为消息卡片插入当前对话流
- **数据预览联动**：上传完成后可在聊天流中的数据卡片里展开预览和查看预处理日志
...
│   │   ├── components/     # React 组件
│   │   │   ├── Header.tsx           # 顶部导航栏
│   │   │   ├── ChatInterface.tsx    # 单栏聊天页容器
│   │   │   ├── chat/                # 聊天 hooks / 消息视图 / SSE 解析
│   │   │   └── ui/                  # UI 组件库
│   │   ├── config/
│   │   │   └── api.ts      # API 配置
│   │   └── App.tsx         # 主应用组件
